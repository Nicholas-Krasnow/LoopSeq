...
# Local batch AA analysis script using CSV configuration
# Usage: bash run_local_batch_analysis.sh [config_file.csv]
# Default config file: sample_config.csv
# This script runs analyses locally without cluster submission
...
if [ ! -f "$CONFIG_FILE" ]; then
    ...
    echo "Usage: bash run_local_batch_analysis.sh [config_file.csv]"
    exit 1
fi
...
run_single_analysis() {
    ...
    local gene_start="$4"
    local gene_length="$5"
    local output_dir="aa_analysis_results_${sample_name}_local"
...
# Main execution
main() {
    echo "Starting LOCAL batch AA analysis from CSV configuration..."
    ...
    # Summary
    echo "=========================================="
    echo "LOCAL BATCH ANALYSIS COMPLETE"
    echo "=========================================="
    ...
    for sample_name in "${failed_samples[@]}"; do
        echo "  - aa_analysis_results_${sample_name}_local/"
    done
    # Also show successful ones
    echo "  - (check for aa_analysis_results_*_local/ directories)"
}
