#!/usr/bin/env python3
"""
Detailed debug script to understand why no mutations are being found.
"""
...
def find_aa_mutations(ref_aa: str, query_aa: str, start_pos: int) -> list:
    ...
    print(f"Comparing AA sequences: ref_len={len(ref_aa)}, query_len={len(query_aa)}")
    print(f"Ref AA start: {ref_aa[:50]}...")
    print(f"Query AA start: {query_aa[:50]}...")
    
    for i in range(min_len):
        if ref_aa[i] != query_aa[i]:
            # Convert to 1-based indexing
            mutations.append((start_pos + i + 1, ref_aa[i], query_aa[i]))
            print(f"  Mutation at position {start_pos + i + 1}: {ref_aa[i]} -> {query_aa[i]}")
