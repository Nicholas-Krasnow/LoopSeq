...
This script compares sp22_dna.fa (protein sequence) and sp24_dna.fa (DNA sequence) 
and identifies amino acid mutations with SP24 as reference.
"""
...
def main():
    # File paths
    sp22_file = "sp22_dna.fa"
    sp24_file = "sp24_dna.fa"
    output_file = "reference_mutations_sp24_vs_sp22.txt"
    
    print("Loading reference sequences...")
    
    # Check if files exist
    if not os.path.exists(sp22_file):
        print(f"Error: {sp22_file} not found!")
        return 1
    ...
    # Load sequences
    try:
        sp22_record = next(SeqIO.parse(sp22_file, "fasta"))
        sp24_record = next(SeqIO.parse(sp24_file, "fasta"))
    except Exception as e:
        print(f"Error loading sequences: {e}")
        return 1
    
    sp22_seq = str(sp22_record.seq).upper()
    sp24_seq = str(sp24_record.seq).upper()
    
    print(f"SP22 sequence length: {len(sp22_seq)}")
    print(f"SP24 sequence length: {len(sp24_seq)}")
    
    # Check if SP22 is already a protein sequence (contains amino acid letters)
    amino_acids = set('ACDEFGHIKLMNPQRSTVWY*')
    sp22_is_protein = all(c in amino_acids for c in sp22_seq)
    
    if sp22_is_protein:
        print("SP22 appears to be a protein sequence (amino acids)")
        sp22_aa = sp22_seq
    else:
        print("Translating SP22 DNA to amino acids (+1 reading frame)...")
        sp22_aa = translate_dna(sp22_seq, reading_frame=1)
    
    # Translate SP24 DNA to amino acids
    print("Translating SP24 DNA to amino acids (+1 reading frame)...")
    sp24_aa = translate_dna(sp24_seq, reading_frame=1)
    
    print(f"SP22 protein length: {len(sp22_aa)}")
    print(f"SP24 protein length: {len(sp24_aa)}")
    
    # Find mutations (SP24 as reference, SP22 as query)
    print("Finding amino acid mutations...")
    mutations = find_aa_mutations(sp24_aa, sp22_aa, reading_frame=1)
    ...
    with open(output_file, 'w') as f:
        f.write("Amino Acid Mutations: SP24 vs SP22 (Reference: SP24)\n")
        f.write("=" * 60 + "\n")
        f.write("Position\tSP24_AA\tSP22_AA\tMutation\n")
        f.write("-" * 60 + "\n")
        ...
        f.write(f"SP22 protein length: {len(sp22_aa)}\n")
        f.write(f"SP24 protein length: {len(sp24_aa)}\n")
        f.write(f"Reading frame: +1\n")
        f.write(f"SP22 input type: {'Protein' if sp22_is_protein else 'DNA'}\n")
